...
class Collection extends EventTarget{
	constructor(config){
		...
		Object.defineProperty(this,'$collectionModelKeyName',{
			configurable: false,
			enumerable: true,
			writable: false,
			value: config.collectionModelKeyName || null,
		});

		Object.defineProperty(this,'$collectionModelKeyValue',{
			configurable: false,
			enumerable: true,
			writable: false,
			value: config.collectionModelKeyValue || null,
		});
...
class StorageQuery{
	...
	async search(filter,options={}){
		options = Object.assign({
			setAbortControllerCallback: null,
			appendCollectionModelKey: true,
		},options);
		const request = {
			entityName: this.$storageEntityName,
			filter,
		};
		if(options.appendCollectionModelKey && this.$collectionModelKeyName !== null && this.$collectionModelKeyValue !== null){
			request.filter = request.filter || {};
			request.filter[this.$collectionModelKeyName] = this.$collectionModelKeyValue;
		}
		return await this.$storage.search(request,options.setAbortControllerCallback);
	}
...
class PersistentCollection extends Collection{
	...
	async $fetch(options={}){
		...
		if(this.$collectionModelKeyName !== null && this.$collectionModelKeyValue !== null){
			request.query = request.query || {};
			request.query[this.$collectionModelKeyName] = this.$collectionModelKeyValue;
		}
	...
	async $save(options={}){
		options = Object.assign({
			...
			additionalPropertiesOnNewModelsOnly: true,
			validate: true,
			validateAll: false,
		},options);
		...
		for(let model of this.$items){
			...
			if((model.$isNew || !_options.additionalPropertiesOnNewModelsOnly) && this.$collectionModelKeyName !== null && this.$collectionModelKeyValue !== null && !_options.ownPropertiesOnly){
				_options.additionalProperties = _options.additionalProperties || {};
				_options.additionalProperties[this.$collectionModelKeyName] = this.$collectionModelKeyValue;
				_options.changesOnly = false;
			}
			await model.$save(_options);
		}
		return true;
	}
	...
	get $storageQuery(){
		return new StorageQuery({
			storage: this.$storage,
			storageEntityName: this.$storageEntityName,
			collectionModelKeyName: this.$collectionModelKeyName,
			collectionModelKeyValue: this.$collectionModelKeyValue,
		});
	}
}
