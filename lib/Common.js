...
class MMModelEvent extends MMEvent{
	constructor(config){
		super(config);
...
class MMCollectionEvent extends MMEvent{
	constructor(config){
		super(config);
		...
		Object.defineProperty(this,'$$trigger',{
			...
			value: (event)=>{
				...
				for(let record of [...listeners[event.type]]){
					if(record.selector !== null && (!(event instanceof MMPropertyChangeEvent) || record.selector != event.selector))
						continue;
...
export { MMEvent, MMSyncEvent,MMPropertyChangeEvent, MMErrorEvent ,MMEventTarget, MMModelEvent, MMCollectionEvent }