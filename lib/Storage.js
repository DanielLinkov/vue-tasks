...
class RestApiJsonClient extends StorageBase{
	...
	async read(request){
		...
		let queryString = '';
		if(request.hasOwnProperty('query') && request.query !== null && request.query !== undefined){
			if(isPlainObject(request.query)){
				queryString = Object.entries(request.query).map(([key,value])=>{
					return `${encodeURIComponent(key)}=${encodeURIComponent(value)}`;
				}).join('&');
			}else if(typeof request.query == 'string'){
				queryString = request.query;
			}else{
				throw new Error("Invalid query type: " + request.query);
			}
			url += '?' + queryString;
		}
		...
		try{
			...
			if(navigator.onLine)
				throw e;
			throw new NetworkError("Navigator is offline");
		}
	}
...
/* 
#TODO:
- facilitate undo operation
- cancel pending requests upon new requests
