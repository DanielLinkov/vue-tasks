...
import { MMEventTarget,MMEvent, MMCollectionEvent, MMSyncEvent } from "./Common.js";

class Collection extends MMEventTarget{
	constructor(config){
		super({
			eventTypes: {
				change: {
		...
		Object.defineProperty(this,'$add',{
			...
			value: (model,atIndex=-1)=>{
				...
				this.$$trigger(new MMCollectionEvent({
					type: 'add',
					target: this,
					model: model,
					index: atIndex,
		...
		Object.defineProperty(this,'$removeWhere',{
			...
			value: (filter)=>{
				...
				if(typeof filter === 'function'){
					...
					throw new Error(`Invalid filter parameter type: ${filter}=>${typeof filter}`);
				_items.forEach(item => item.$setCollection(null));
				return _items;
			}
		});

		Object.defineProperty(this,'$deleteWhere',{
			configurable: false,
			enumerable: false,
			writable: false,
			value: (filter)=>{
				...
				if(typeof filter === 'function'){
					...
					_items.forEach(item => item.$delete({
						$storage: this.$storage,
						$storageEntityName: this.$storageEntityName,
					}));
...
class PersistentCollection extends Collection{
	...
	async $fetch(options={}){
		...
		try{
			...
			this.$isNew = false;
			this.$$trigger(new MMSyncEvent({
				target: this,
				targetUpdated: isUpdated,
				syncType: MMSyncEvent.SYNC_TYPE_READ
			}));
			return isUpdated;
		}catch(err){
			throw {
