...
import { EventTarget,Event as _Event, CollectionEvent, SyncEvent } from "./Common.js";
...
class Collection extends EventTarget{
	constructor(config){
			...
			value: (model,atIndex=-1,config={})=>{
				...
				if(!(model instanceof this.$modelClass)){
					...
					model = new this.$modelClass(model,{
						updatePersistentState: !!config.updatePersistentState
					});
					if($key !== null){
						model.$setKey($key);
					}
		...
		Object.defineProperty(this,'$deleteOne',{
			...
			writable: false,
			value: async (filter)=>{
				const item = this.$removeOne(filter);
				if(item === null)
					return null;
		...
		Object.defineProperty(this,'$deleteWhere',{
			...
			writable: false,
			value: (filter)=>{
				let _items = this.$removeWhere(filter);
				_items.forEach(item => {
					item.$delete({
						$storage: this.$storage,
						$storageEntityName: this.$storageEntityName,
