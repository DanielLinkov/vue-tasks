...
import { EventTarget } from "./Common.js";
	...
	constructor(config){
		...
		Object.defineProperty(this,'$removeOne',{
			...
			value: (filter)=>{
				...
				if(ind == -1)
					return null;
				const model = items.splice(ind,1)[0];
				model.$setCollection(null);
				return model;
			}
		});
		...
		Object.defineProperty(this,'$deleteOne',{
			...
			value: async (filter)=>{
				...
				if(ind == -1)
					return null;
				const item = items.splice(ind,1)[0];
	...
	create(config=null){
		...
		const CollectionClass = class extends Collection{
			constructor(_config={}){
				...
				super(__config);
				Object.seal(this);
				if(__config.hasOwnProperty('onCreated') && typeof __config.onCreated === 'function')
					__config.onCreated(this);
			}
		}
