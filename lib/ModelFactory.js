...
import { EventTarget,Event as _Event,PropertyChangeEvent,SyncEvent,ModelEvent,ValidationEvent } from "./Common.js";
...
class Model extends EventTarget{
	constructor(config,defaults){
		...
		Object.defineProperty(this,'$revert',{
			...
			writable: false,
			value: () => {
				console.log('revert',propPersistentState);
		...
		Object.defineProperty(this,'$validate',{
			...
			value: (propName=null) => {
				...
				for(let propName of _propNames){
					...
					(propValidators[propName] || []).forEach(async (callback)=>{
						try{
							let ret;
							ret = callback(val,this,(err)=>{
				...
				if(propName === null){	//All properties validated
					...
					return _propTransforms;	//Return transforms
					...
					return _propTransforms.hasOwnProperty(propName) ? _propTransforms[propName] : val;	//Return transform for specific property
				}
			}
		});
...
class PersistentModel extends Model{
	...
	async $delete(options={}){
		...
		if(await options.$storage.delete(request)){
			this.$$trigger(new SyncEvent({
				target: this,
