...
import { MMEventTarget,MMEvent,MMPropertyChangeEvent,MMSyncEvent,MMModelEvent } from "./Common.js";
...
class PersistentModel extends Model{
	...
	async $save(options={}){
		...
		this.$$trigger(new MMSyncEvent({
			target: this,
			targetUpdated: isUpdated,
			syncType: MMSyncEvent.SYNC_TYPE_WRITE
		}));
		return true;
	}
	async $delete(options={}){
		...
		if(await options.$storage.delete(request)){
			this.$$trigger(new MMSyncEvent({
				target: this,
				syncType: MMSyncEvent.SYNC_TYPE_DELETE
			}));
			return true;
		};
		//#TODO: Error handling
