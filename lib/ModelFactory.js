...
class Model{
	constructor(config,defaults){
		...
		let propMethods	= {};	//List of property methods
		...
		if(config.hasOwnProperty('methods')){
			...
			for(let methodName of Object.keys(config.methods)){
				...
				if(config.methods[methodName].length === 0){
					Object.defineProperty(propMethods,methodName,{
						configurable: false,
						enumerable: true,
						get: () => {
...
class PersistentModel extends Model{
	...
	async $save(options={}){
		...
		try{
			if(options.updateFromStorage && json !== null)
	...
	create(config=null){
		...
		const ModelClass = class extends Model{
			constructor(defaults={}){
				config.uuid = ModelFactory.uuidv4();
				super(config,defaults);
				Object.seal(this);
