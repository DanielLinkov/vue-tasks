...
class Model{
	constructor(config,defaults){
		...
		const propErrors = {};	//List of property errors
		const propValidated = {};	//List of property validated flags
		const propState = {};	//Keeps track of property values
		const propPersistentFlags = {};	//Only serialize/unserialize properties with true flag
		const propPersistentState = {};	//Keeps track of property persistent values (as they are in storage)
		...
		Object.keys(config.persistent || {}).forEach((propName)=>{
			if(typeof config.persistent[propName] !== 'boolean')
