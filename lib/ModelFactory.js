...
class Model{
	constructor(config,defaults){
		...
		const watchCallbacks = {};	//List of property watch callbacks
		...
		const computed	= {};	//List of property methods
			...
			value: (options={}) => {
				...
				for(let propName of [...new Set(_propNames)]){	//Remove duplicates
					...
					if(propNameParts.length == 2){	//Access path given
						if(!this.hasOwnProperty(propNameParts[0]))
							throw new Error(`Missing property: ${propNameParts[0]}`);
						if(!this[propNameParts[0]].hasOwnProperty(propNameParts[1]))
							throw new Error(`Missing property: ${propNameParts[0]}.${propNameParts[1]}`);
						data[`${propNameParts[0]}.${propNameParts[1]}`] = this[propNameParts[0]][propNameParts[1]];
					}else{
